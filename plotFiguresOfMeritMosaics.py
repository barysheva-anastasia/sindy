...
from core.plot import colors as projectColors  # Single source of truth for the palette, so the
# mosaics match the trajectory plots. NOTE: importing 'core.plot' also applies its seaborn palette
# globally, since that module calls 'sns.set_palette' at import time.

maxNumVarsForLegend = len(projectColors)  #!!!!!# Above this many variables the legend is dropped.
# Two reasons, and the palette length is the honest bound for both. Colors cycle (see
# 'i % colors_cnt' below), so past len(projectColors) == 6 the legend maps distinct variables onto
# the same swatch and states something false rather than merely being crowded. And a long legend
# breaks the layout: at 27 variables it is taller than a row of the 6 x 3 grid, 'tight_layout' gives
# up with 'Tight layout not applied' and leaves matplotlib's default margins, and since the mosaic
# is saved with bbox_inches='tight' the PNG is then cropped to that smaller occupied region -- about
# 20% under the nominal 18 x 24 inches, shrinking every subplot with it. Measured: 21 variables ->
# 2722 x 3606 px, 27 and up -> 2170 x 2868 px.
...
def plotFoMSubplot_fn(y, x, ax, titleStr, yLims, varNames, c, thisMark, legendLoc='',
    ...
    legendLoc : str, default = ''. '' -> no legend. Ignored, ie still no legend, when there are more
                than 'maxNumVarsForLegend' variables.
    ...
    for i in range(numVars):
        ax.scatter(x + i * 0.05 + offset, y[:, i],  marker=thisMark, color=c[i%colors_cnt], s=12, 
                   label=varNames[i])
    ax.set_title(titleStr, fontsize=12, fontweight='bold')
    if len(legendLoc) > 0 and numVars <= maxNumVarsForLegend:
...
def plotFiguresOfMeritMosaics_fn(d):
    ...
    for traj in range(numTrajTrain):
        ...
        tag = 'Train ' + str(traj) + '. ' 
        thisMark = 'o'
        colorList = projectColors  # was ['r', 'b', 'g', 'm', 'c']. 'plotFoMSubplot_fn' already
        # cycles with i % len(colorList), so the length may differ from 'markerList'.
        
        fig, axs = plt.subplots(nrows=6, ncols=3, sharex=True, figsize=(18, 24))
        ...
        # Save figure to file
        outputFilename = d.get('outputFilename', 'outputFile')
        figFilename = outputFilename + '_FoM_mosaic_traj' + str(traj) + '.png'
        fig.savefig(figFilename, dpi=150, bbox_inches='tight')
        print(f'Saved FoM mosaic for trajectory {traj} to {figFilename}')
